...
public abstract class AbstractEvaluator {
	...
	protected String listOfIntsToString(List<Long> list) {
		StringBuilder sb = new StringBuilder();
		for (Long id : list) {
			sb.append(id + "\t");
		}
	...
	private List<Long> getMostDifficultUsers(AbstractRecommenderFairEvaluator evaluator, int limit) {
		Map<Long, Double> averageAbsoluteErrors = new HashMap<Long, Double>();
		for (Long userID : evaluator.estimatesForUsers.keySet()) {
		...
		List<Entry<Long, Double>> list = sortByLargest(averageAbsoluteErrors, limit);
		return limit(limit, list);
	}
	
	private List<Long> getMostDifficultItems(AbstractRecommenderFairEvaluator evaluator, int limit) {
		Map<Long, Double> averageAbsoluteErrors = new HashMap<Long, Double>();
		for (Long itemID : evaluator.estimatesForItems.keySet()) {
	...
	private List<Long> getLeastDifficultUsers(AbstractRecommenderFairEvaluator evaluator, int limit) {
		Map<Long, Double> averageAbsoluteErrors = new HashMap<Long, Double>();
		for (Long userID : evaluator.estimatesForUsers.keySet()) {
			List<Double> estimateAbsoluteErrors = evaluator.estimatesForUsers.get(userID);
			Double averageAbsoluteError = average(estimateAbsoluteErrors);
		...
		List<Entry<Long, Double>> list = sortBySmallest(averageAbsoluteErrors, limit);
		return limit(limit, list);
	}
	
	private List<Long> getLeastDifficultItems(AbstractRecommenderFairEvaluator evaluator, int limit) {
		Map<Long, Double> averageAbsoluteErrors = new HashMap<Long, Double>();
		for (Long itemID : evaluator.estimatesForItems.keySet()) {
			List<Double> estimateAbsoluteErrors = evaluator.estimatesForItems.get(itemID);
			Double averageAbsoluteError = average(estimateAbsoluteErrors);
	...
	private List<Long> limit(int limit, List<Entry<Long, Double>> list) {
	...
	private List<Entry<Long, Double>> sortByLargest(Map<Long, Double> averageAbsoluteErrors, int limit) {
		// Convert Map to List
		List<Entry<Long, Double>> list = new LinkedList<Entry<Long, Double>>(averageAbsoluteErrors.entrySet());
	...
	private List<Entry<Long, Double>> sortBySmallest(Map<Long, Double> averageAbsoluteErrors, int limit) {
		// Convert Map to List
		List<Entry<Long, Double>> list = new LinkedList<Entry<Long, Double>>(averageAbsoluteErrors.entrySet());
